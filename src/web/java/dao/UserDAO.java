...
public class UserDAO {
	...
	public void deleteUserById(String id) {
		...
		String query1 = "delete from comment where rating_user_id = ?";
		try {
			System.out.print(Integer.parseInt(id));
			conn = new ConnectDB().getDBConnection();
	...
	public float revenueTotal() {
		String query = "select sum(total) from ordertotal";
		float total =0;
		try { 
			conn = new ConnectDB().getDBConnection();
			ps = conn.prepareStatement(query);
	...
	public List<OrderSingle> revenueStaticEachProduct() {
		List<OrderSingle> orderSingles = new ArrayList<OrderSingle>();
		String query = "select product_id, cast(sum(price) as float ), sum(number) from laptopweb.ordersingle group by product_id;";
		try { 
			conn = new ConnectDB().getDBConnection();
			ps = conn.prepareStatement(query);
			rs = ps.executeQuery();
