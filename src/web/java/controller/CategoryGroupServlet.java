...
public class CategoryGroupServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		request.setAttribute("currentCategory", request.getParameter("id"));
		request.setAttribute("topfiveproducts", new ProductDAO().topfiveproducts(request.getParameter("id")));
		request.setAttribute("categorySames", new CategoryDAO()
				.getCategorySames(new CategoryDAO().getCollectionNameById(request.getParameter("id"))));
